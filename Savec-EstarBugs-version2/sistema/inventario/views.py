...
from .forms import LoginFormulario, ClienteFormulario, ProductoFormulario, EmitirFacturaFormulario, DetallesFacturaFormulario, EmitirPedidoFormulario, DetallesPedidoFormulario, ProveedorFormulario, UsuarioFormulario, NuevoUsuarioFormulario, ClaveFormulario
from .models import Usuario, Opciones, Producto, Cliente, Factura, DetalleFactura, Proveedor, Pedido, DetallePedido
from .funciones import productoTieneIva, sacarIva, ivaActual, obtenerProducto, usuarioExiste, manejarArchivo, render_to_pdf
...
class Login(View):
    def post(self, request):
        ...
        if form.is_valid():
            ...
            # Verificar la autenticación con la contraseña en texto plano
...
class Panel(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        contexto = {'usuario': request.user.username,
                   ...
                   'fecha':  date.today(),
                   'productosRegistrados' : Producto.numero_registrados(),
...
from .forms import UsuarioFormulario, ClaveFormulario
    ...
    def get(self, request, modo, p):
        if modo == 'editar':
            ...
            if editandoSuperAdmin:
                ...
                form = UsuarioFormulario(initial={
            ...
            contexto = {
                'form': form,
                'modo': request.session.get('perfilProcesado'),
                'editar': 'perfil',
    ...
    def post(self, request, modo, p):
        if modo == 'editar':
            ...
            if form.is_valid():
                ...
                perf.username = form.cleaned_data['username']
                perf.first_name = form.cleaned_data['first_name']
                perf.last_name = form.cleaned_data['last_name']
                perf.email = form.cleaned_data['email']

                perf.save()

                messages.success(request, 'Perfil de ID %s actualizado exitosamente.' % p)
            ...
            if form.is_valid():
                ...
                if clave_nueva == repetir_clave:
                    ...
                    messages.error(request, "La clave nueva y su repetición deben coincidir")
                    return HttpResponseRedirect(f"/inventario/perfil/clave/{p}")
...
# Muestra el formulario de un producto específico para editarlo----------------------------------#
class EditarProducto(LoginRequiredMixin, View):
    ...
    def post(self, request, p):
        ...
        if form.is_valid():
            ...
            categoria = form.cleaned_data['categoria']
            tiene_iva = form.cleaned_data['tiene_iva']
            disponible = form.cleaned_data['disponible']  # Agregado el campo disponible
            ...
            prod.disponible = disponible  # Actualizado el campo disponible
            prod.save()
            ...
            # De lo contrario, mostrará el mismo formulario
            return render(request, 'inventario/producto/agregarProducto.html', {'form': form})
